...
### This Branch: `world-with-fake-iran`
- `splice_iran_synthetic_data()` in `data.py` replaces real Iran data from 2026-02-27 onward with synthetic forecast values
- Called unconditionally in `main.py` after `get_aggregate_data()`
- Requires `data/iran_synthetic/iran_synthetic.parquet` to exist (generated by `scripts/generate_iran_synthetic.py` on the `iran-synthetic-generation` branch)
