...
**Bidirectional overlay machinery (generalized).** The per-tile bidirectional appliers are model-agnostic: `compute_country_shares(training_df, flag_column=...)`, `subtract_lift_from_training(df, flag_column=..., sentinel_attr=...)`, and `add_lift_to_forecast(df, population_value=...)`. They key off a boolean segment column (`fenix_android` for mobile `m`; `modern_windows` for desktop `l`) and allocate a single daily aggregate series across country tiles by trailing DAU share. `m` and `l` are thin wrappers over these. **The desktop `desktop_overlay` spec type (`load_overlay_spec`) is the reuse path for future desktop tailwinds** (e.g. MozillaOnline `o`) — see `data-official/2026-07/mozillaonline/WIRING_HANDOFF.md`. Each overlay must pass a distinct `sentinel_attr` so two overlays can stack on the same desktop training frame.

**Disabling an overlay for a one-off run**: `python scripts/run_main.py --no-marketing-lift ...` (mobile `m`) or `--no-launch-on-login ...` (desktop `l`). Default behavior is to apply an overlay whenever its spec (`.../marketing/marketing.json`, `.../launch_on_login/lol.json`) has `applies_to_forecast_start == forecast_start_date`.
