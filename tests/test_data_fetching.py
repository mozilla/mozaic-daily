...
def test_get_queries_dau_only_flag():
    """Verify dau_only flag returns only DAU metrics for all platforms/sources.

    Should reduce from 12 queries (4 metrics × 3 sources) to 3 queries (DAU × 3 sources).
    """
    # Get all queries (default)
    all_queries = get_queries("'US', 'DE'", dau_only=False)

    # Count total queries in default mode
    total_all = sum(
        len(metrics)
        for sources in all_queries.values()
        for metrics in sources.values()
    )
    assert total_all == 12, f"Expected 12 total queries, got {total_all}"

    # Get DAU-only queries
    dau_queries = get_queries("'US', 'DE'", dau_only=True)

    # Count total queries in DAU-only mode
    total_dau = sum(
        len(metrics)
        for sources in dau_queries.values()
        for metrics in sources.values()
    )
    assert total_dau == 3, f"Expected 3 DAU-only queries, got {total_dau}"

    # Verify only DAU metric present
    for platform in dau_queries.values():
        for source in platform.values():
            assert list(source.keys()) == ['DAU'], (
                f"Expected only 'DAU' metric when dau_only=True, got {list(source.keys())}"
            )


def test_get_queries_dau_only_with_testing_mode():
    """Verify dau_only and testing_mode flags work together correctly.

    testing_mode already filters to Desktop Glean DAU only.
    dau_only should not conflict with this.
    """
    # Testing mode alone: Desktop Glean DAU only
    testing_queries = get_queries("'US', 'DE'", testing_mode=True, dau_only=False)
    assert 'DAU' in testing_queries['desktop']['glean']
    assert len(testing_queries['desktop']['glean']) == 1

    # Both flags: should still be Desktop Glean DAU only
    both_flags_queries = get_queries("'US', 'DE'", testing_mode=True, dau_only=True)
    assert 'DAU' in both_flags_queries['desktop']['glean']
    assert len(both_flags_queries['desktop']['glean']) == 1


def test_get_aggregate_data_with_dau_only_executes_correct_count(mocker):
    """Verify that dau_only flag results in only 3 BigQuery calls (not 12).
    ...
    # Verify only 3 queries were executed (Desktop Glean DAU, Desktop Legacy DAU, Mobile Glean DAU)
    assert mock_client.query.call_count == 3, (
        f"Expected 3 queries with dau_only=True, got {mock_client.query.call_count}"
    )
