...
Upload with `gcloud storage cp -r` (parallel, no macOS bug). Run long uploads in the
background and report progress; `data-official/$CYCLE/` alone was 9.4 GB in August and the
scan pickles 67 GB, so expect an hour or more in total.
...
Counts must match exactly; bytes within 1% (GCS `du` reports object bytes, local `du`
reports blocks). Spot-check one large pickle's size with `gcloud storage ls -l`. Record the
numbers in the cycle `_index.md` "Present vs Archived" section.
...
Output a table: file, line, referenced path, status of that path after the prune (archived /
deleted / retained). **Do not edit any of these files.** Updating them is a separate task for a
...
- **Keep** the canonical parquets + sidecars for every retained cycle (the notebooks read
  them), the `REVERT_*` directories while their cycle is live, any parquet a canonical
  notebook still loads, and every `.meta.json` / `parameters.json` (the legible record of a
  search without its blobs).
- **Scan exhaust**: `rm -rf research/param-scans/<search>/results/` (or the probe dirs) after
  their pickles verified in GCS; tracked scores/FINDINGS stay.
...
"currently on disk". Re-run `pytest -q`. Commit on `clean-slate` in two commits as before
(prune; doc pass) and push.
