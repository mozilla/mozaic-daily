...
class MozaicDailyFlow(FlowSpec):
    ...
    # Check https://docs.metaflow.org/api/step-decorators/kubernetes for details on @kubernetes decorator
    # @kubernetes(
    #     image="registry.hub.docker.com/brwells78094/mozaic-daily:v0.0.2", 
    #     cpu=1,
    #     memory=16384
    # )
    @step
    def load(self):
        ...
        # print("ADC project seen by worker:", adc_project)
        # print("Credential type:", type(creds).__name__)
        # print("Service account email (if available):", getattr(creds, "service_account_email", None))

        # # Trivial query to test perms
        # bq = bigquery.Client(project="moz-fx-data-bq-data-science", credentials=creds)
        # q = bq.query("SELECT 1 AS x")
        # print(list(q.result()))
