...
its own code so the two can be sized and audited independently. Registry `name` is `launch_on_login_new_users`;
the directory and spec filename keep the historical `launch_on_login/lol.json` layout so August's and July's
...
`../../2026-08/launch_on_login/`; only `applies_to_forecast_start` moved to 2026-09-02. Nothing was re-measured:
...
| file | role |
|---|---|
| `lol.json` | the spec, gated on `applies_to_forecast_start: 2026-09-02` |
