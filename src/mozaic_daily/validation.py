# -*- coding: utf-8 -*-
"""Output validation for mozaic-daily forecast DataFrames.

This module validates forecast DataFrames before uploading to BigQuery.
Validation checks:
- Column presence and types match BigQuery schema
- String format validation (timestamps, dates, git hashes, JSON)
- Row count validation (all countries, dates, segments present)
- Null value checks for expected metric/date combinations
- Duplicate row detection

Functions:
- validate_output_dataframe(): Main validation entry point
- _get_bigquery_fields(): Fetches BigQuery schema
- _check_*(): Individual validation functions
"""
