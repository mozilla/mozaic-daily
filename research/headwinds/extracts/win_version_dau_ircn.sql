-- IR and CN carve-out of the Windows-version split (same date design).
-- Needed because Iran's shutdown recovery (2026-05-26) and the MozillaOnline
-- CN migration both land inside the Apr->Jul measurement window and would
-- otherwise be attributed to Win10 transition effects. `country` is the
-- table's first clustering key, so this filtered pull is cheap.
--
...
GROUP BY submission_date, country, os_bucket
ORDER BY submission_date, country, os_bucket
