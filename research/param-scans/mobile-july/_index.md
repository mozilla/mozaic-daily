...
- `mobile_july_sensitivity.ipynb` — round-1 sensitivity notebook: one plot per knob
...
- `seam_smoothing.ipynb` — removing the actuals→forecast **kink** in the 28d-MA. **Terminology:**
...
- `scripts/mobile_sensitivity.py` — pure scorer (extraction → canonical `display_ma`
  seam MA → headwind daily-anchor convention → net vs baseline). Tested in
  `tests/test_mobile_sensitivity.py`.
