...
| `round{1,2,3}/*/run.log`, `round*.log`, `sampling.log` | Per-probe and driver logs. |
| `round*/*/*/parameters.json`, `*.meta.json` | Sidecars recording exactly what each config was and produced. |
...
- **Present (on disk, tracked):** all `FINDINGS.md`, `*_scores.csv`, logs, `parameters.json` /
  `*.meta.json` sidecars, the notebook, the exported curve CSV, the plot, and the generator scripts.
  The search is fully legible from these alone.
- **Blobs (~40G, gitignored):** per-config forecast `*.parquet`, `mozaic_objects.*.pkl`, and
  `_rawcache/` (633M). **Archive to GCS and delete per the cycle button-down lifecycle** — these are
  regenerable but expensive (each round is hours of Prophet fits).
